...
parser.add_argument('--e', type=int, default=30000, help='Number of samples to process')
...
analytics = ll_torch.NLL(alphas,a=1,b=1,t=91.1876**2)
...
def run_pypy_script(pypy_script_path, n_samp, asif):
    ...
    filename = f"thrust_e{n_samp}_A{asif}_LL.csv"
    ...
    try:
        ...
        if os.path.exists("thrust_LL.csv"):
            os.rename("thrust_LL.csv", filename)
        else:
...
# Generate data once
pypy_script_path = os.path.expanduser('dire_LL.py')
