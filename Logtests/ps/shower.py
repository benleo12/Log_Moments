...
# Thrust distribution formula as a function
def p_tau(tau):
    prefactor = (2.0 * alpha_s) / (3.0 * np.pi * tau) * (-4 * np.log(tau))
    exponent = -2.0 * alpha_s * (2.0 * (np.log(tau))**2) / (3.0 * np.pi)
    return prefactor * np.exp(exponent)

# Generate tau values, avoiding 0 to prevent division by zero or log(0)
tau_values = np.linspace(0.0001, 1.0, 10000)  # Upper limit set to 0.5 for better visualization
p_values = p_tau(tau_values)

# Now plot the theoretical distribution
fig = plt.figure(figsize=(10, 6))
ax = fig.add_subplot(1,1,1)
ax.plot(tau_values, p_values, label='Theoretical Thrust Distribution')
ax.set_yscale('log')
ax.set_xscale('log')
# Overlay with actual thrust distribution
# Assuming 'thrust_values' is a list of thrust values from the previous operations
logbins = np.logspace(-4,0.0,num=50)
print("logbins=",logbins)
plt.hist(thrust_values, bins=logbins, alpha=0.75, density=True, label='Simulated Thrust Distribution')  # Using density=True for probability density
