...
alphas = AlphaS(91.1876,0.118,order=-1)
hardxs = eetojj(alphas)
shower = Shower(alphas,t0=0.1)
...
lambda_0 = -1.0601158
lambda_1 = 0.059548471122
lambda_2 = -0.059780031

def weight(t):
    return  (2*CNLL2*np.log(t)+CNLL1)/(2*(CNLL2-lambda_2)*np.log(t)+(CNLL1-lambda_1))*np.exp(-1 -lambda_0 - lambda_1 * np.log(t) - lambda_2 * np.log(t)**2)
...
tau_values = np.linspace(0.0001, 1.0, 1000)  # Upper limit set to 0.5 for better visualization
...
ax.plot(tau_values, pLL_values, label='LL Theoretical Thrust Distribution', color='blue')
ax.plot(tau_values, pNLL_values, label='NLL Theoretical Thrust Distribution', color='green')
ax.set_yscale('log')
ax.set_xscale('log')
# Overlay with actual thrust distribution
# Assuming 'thrust_values' is a list of thrust values from the previous operations
logbins = np.logspace(-4,0.0,num=100)
#print("logbins=",logbins)
# Calculating weights for each thrust value
weights = weight(thrust_values)

# Weighted histogram
plt.hist(thrust_values, bins=logbins, weights=weights, alpha=0.5, density=True, label='Weighted Simulated Distribution', color='black')

# Unweighted histogram
plt.hist(thrust_values, bins=logbins, alpha=0.5, density=True, label='Unweighted Simulated Distribution', color='red')
