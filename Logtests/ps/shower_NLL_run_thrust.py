...
def torch_quad(func, a, b, num_steps=10000000, func_mul=None, func_mul2=None):
    x = torch.logspace(np.log10(a), np.log10(b), steps=num_steps, dtype=torch.float32)
...
def anal_NLL(tau):
    return analytics.R_SL(tau)-analytics.logF(tau)
...
#CNLL1 = torch_quad(wNLL, min_tau, max_tau, func_mul=analytics.R_L)
#CNLL2 = torch_quad(wNLL, min_tau, max_tau, func_mul=anal_NLL)
#CNLL2 = torch_quad(wNLL, min_tau, max_tau, func_mul=analytics.R_SL)
#CNLL3 = torch_quad(wNLL, min_tau, max_tau, func_mul=analytics.logF)
...
def run_pypy_script(pypy_script_path):
    ...
    try:
        subprocess.check_call(['pypy', pypy_script_path, '-e', str(n_samp)])
...
def integral_equation_1_direct(lambda_0, lambda_1, lambda_2, tau_i, n_samp):
    ...
    expon = torch.exp(lambda_0 - lambda_1*(analytics.R_SL(tau_i)-analytics.logF(tau_i)) -  lambda_2*analytics.R_L(tau_i))
    moment = 1
...
def integral_equation_2_direct(lambda_0,lambda_1, lambda_2, tau_i, n_samp):
    ...
    #moment = analytics.R_SL(tau_i)-analytics.logF(tau_i) 
    integral = mc_integration(expon*part*moment, tau_i, n_samp)/mc_integration(expon*part, tau_i, n_samp)
...
lambda_2 = torch.tensor([0.2546926*0], requires_grad=True)
...
for step in range(1000000):
    ...
    filtered_tau_0 = tau_i[(tau_i <= 10**-13)]
