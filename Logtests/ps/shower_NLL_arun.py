...
parser.add_argument('--n_samp', type=int, default=100, help='Number of samples to process')
...
# partC = 100*(analytics.Rp(tau))/tau
# exponC = np.exp(  -  100*(analytics.R_L(tau)))
# partC = 100*(analytics.RNNLLp(tau))/tau
# exponC = np.exp(  - 100*(analytics.R_SL(tau)))
# partC = 10*(-analytics.FpF(tau))/tau
# exponC = np.exp(  -  10*(-analytics.logF(tau)))
 return partC*exponC 
...
# Calculate `int_check` over the range
values = [int_check(10**tau) for tau in taus]


# Plot the results
plt.plot(taus, values)
#plt.gca().set_yscale('log')
plt.xlabel('tau')
plt.ylabel('int_check(tau)')
plt.title('Plot of int_check from 0.001 to 1')
plt.grid(True)
plt.show()

intC, _ = quad(lambda t: int_check(t), 1e-3, 1.)

print("int check = ",intC)
...
def integral_equation_1_direct(lambda_0, lambda_1, lambda_2, tau_i, n_samp):
    ...
    RNNLLpt = analytics.RNNLLp(tau_i) #Banfi, A1 is 2CF, ignore middle line
# CHECK: (derivative of part)*exp from 0 to 1
    part = (CNLL2*Rpt + CNLL1*(RNNLLpt-FpFt))/((CNLL2-lambda_2)*Rpt+(CNLL1-lambda_1)*(RNNLLpt-FpFt))
    expon = torch.exp(-lambda_0 - lambda_1 * (analytics.R_SL(tau_i)-logFt) - lambda_2 * analytics.R_L(tau_i))
