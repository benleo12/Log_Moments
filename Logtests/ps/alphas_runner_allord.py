...
def run_nllc_script(asif, lambda_2):
    command = [
        'python3', 'shower_NLLc_thrust.py',
        '--asif', str(asif),
        '--e', str(evs),
        '--lam_LL', str(lambda_2)
    ]
    try:
        ...
        print(f"An error occurred while running the NLLc script with asif={asif}, lambda_2={lambda_2}: {e}")
...
for asif in asif_values:
    ...
    if os.path.exists(params_ll_filename):
        with open(params_ll_filename, 'r') as f:
            ...
            ll_results.append((asif, lambda_2_value))

# Save LL results to a file
with open('ll_params.txt', 'w') as f:
    for asif, lambda_2_value in ll_results:
...
lambda_2_plot_values = [result[1] for result in ll_results]

# Plot lambda_2 vs. asif
plt.plot(asif_plot_values, lambda_2_plot_values, marker='o', label='LL Data')
...
nllc_results = []
for asif, lambda_2_value in ll_results:
    print("lambda_2_value:", lambda_2_value)
    #TODO: Uncomment the following line, or understand why it affects the results
    #run_nllc_script(asif, lambda_2_value)
    run_nllc_script(asif, 0.0)
    params_nllc_filename = f'params_NLLc_{evs}.txt'
    if os.path.exists(params_nllc_filename):
        with open(params_nllc_filename, 'r') as f:
            lines = f.readlines()
            lambda_2_value_nllc = float(lines[0].split(":")[1].strip())
...
with open('nllc_params.txt', 'w') as f:
    for asif, lambda_2_value_nllc in nllc_results:
        f.write(f"asif: {asif}, lambda_2 (NLLc): {lambda_2_value_nllc}\n")
