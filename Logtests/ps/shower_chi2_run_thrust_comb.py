...
def run_pypy_script(pypy_script_path,asif,n_samp,piece):
    ...
    flags = ['-e',str(n_samp),'-A',str(asif),'-b','1','-C','1.0','-x',str(piece)]
...
min_tau = torch.min(filtered_tau_0)*10
max_tau = torch.max(filtered_tau_0)/10
...
defrate = 0.1
if args.piece == 'll':
    optimizer = optim.RAdam([{'params': lambda_1, 'lr': defrate}])
if args.piece == 'nllc' or args.piece == 'nll1':
    optimizer = optim.RAdam([{'params': lambda_1, 'lr': defrate},{'params': lambda_2, 'lr': defrate}])
