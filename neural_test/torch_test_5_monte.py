...
tau_i = torch.linspace(1e-5, 1, steps=1000)
q_tau_i = (2 * alpha_s / (3 * torch.pi)) * (-4 * torch.log(tau_i) / tau_i)  # Example generation

# Manual integration function using Riemann sum
def manual_integration(integrand, tau_values):
    delta_tau = tau_values[1:] - tau_values[:-1]
    integral = 0.5 * torch.sum((integrand[1:] + integrand[:-1]) * delta_tau)
    return integral

# Define the integral equations using the dataset directly
def integral_equation_1_direct(lambda_0, lambda_1, lambda_2):
    integrand = q_tau_i * torch.exp(-1 - lambda_0 - lambda_1 * torch.log(tau_i) - lambda_2 * torch.log(tau_i)**2)
    integral = manual_integration(integrand, tau_i)
    return integral - 1

def integral_equation_2_direct(lambda_0, lambda_1, lambda_2):
    integrand = q_tau_i * torch.exp(-1 - lambda_0 - lambda_1 * torch.log(tau_i) - lambda_2 * torch.log(tau_i)**2) * torch.log(tau_i)
    integral = manual_integration(integrand, tau_i)
    return integral + np.sqrt(3) * torch.pi / (4 * np.sqrt(alpha_s))

def integral_equation_3_direct(lambda_0, lambda_1, lambda_2):
    integrand = q_tau_i * torch.exp(-1 - lambda_0 - lambda_1 * torch.log(tau_i) - lambda_2 * torch.log(tau_i)**2) * torch.log(tau_i)**2
    integral = manual_integration(integrand, tau_i)
...
optimizer = optim.Adam([lambda_0, lambda_1, lambda_2], lr=0.01)
