...
def integral_equation_1_direct(lambda_0, lambda_1, lambda_2,sum_tau_i,n_samp):
    ...
    integral = mc_integration(expon*part, sum_tau_i, n_samp)
    return integral - CNLL0

def integral_equation_2_direct(lambda_0, lambda_1, lambda_2,sum_tau_i,n_samp):
    part = (2*(kap_2+lambda_2)*torch.log(sum_tau_i)+(kap_1+lambda_1))/(2*kap_2*torch.log(sum_tau_i)+kap_1)
...
def integral_equation_3_direct(lambda_0, lambda_1, lambda_2,sum_tau_i,n_samp):
    part = (2*(kap_2+lambda_2)*torch.log(sum_tau_i)+(kap_1+lambda_1))/(2*kap_2*torch.log(sum_tau_i)+kap_1)
...
for step in range(1000000):
    ...
    filtered_sum_tau_i = filtered_sum_tau_i[(filtered_sum_tau_i  >= min_tau) & (filtered_sum_tau_i  <= max_tau)]

    #if step % (5000) == 0:
    #    print("iterations = ",iterations_count)

    optimizer.zero_grad()
    loss_1 = integral_equation_1_direct(lambda_0, lambda_1, lambda_2, filtered_sum_tau_i, n_samp)**2
    loss_2 = integral_equation_2_direct(lambda_0, lambda_1, lambda_2, filtered_sum_tau_i, n_samp)**2
    loss_3 = integral_equation_3_direct(lambda_0, lambda_1, lambda_2, filtered_sum_tau_i, n_samp)**2
