...
#kap_1 = -np.sqrt(3)*np.pi/4/np.sqrt(alpha_s)
kap_2 = 4*alpha_s/(3*torch.pi)/2
...
def manual_integration(integrand, tau_values, lambda_2):
...
def integral_equation_1_direct(lambda_0, lambda_1, lambda_2):
    integrand = (kap_2+lambda_2)/kap_2*torch.exp(-1 - lambda_0 - lambda_1 * torch.log(tau_i) - lambda_2 * torch.log(tau_i)**2)
    integral = manual_integration(integrand, tau_i, lambda_2)
    return integral - 1

def integral_equation_2_direct(lambda_0, lambda_1, lambda_2):
    integrand = (kap_2+lambda_2)/kap_2*torch.exp(-1 - lambda_0 - lambda_1 * torch.log(tau_i) - lambda_2 * torch.log(tau_i)**2) * torch.log(tau_i)
    integral = manual_integration(integrand, tau_i,lambda_2)
    return integral + np.sqrt(3) * torch.pi / (4 * np.sqrt(alpha_s))

def integral_equation_3_direct(lambda_0, lambda_1, lambda_2):
    integrand = (kap_2+lambda_2)/kap_2*torch.exp(-1 - lambda_0 - lambda_1 * torch.log(tau_i) - lambda_2 * torch.log(tau_i)**2) * torch.log(tau_i)**2
    integral = manual_integration(integrand, tau_i, lambda_2)
    return integral - (3 * torch.pi) / (4 * alpha_s)

# Initialize the Lagrange multipliers
lambda_0 = torch.tensor([-1.0], requires_grad=True)
lambda_1 = torch.tensor([(-0 * np.sqrt(alpha_s)/(torch.pi * np.sqrt(3)))*0.1], requires_grad=True)
lambda_2 = torch.tensor([4 * alpha_s / (3 * torch.pi)*0.1], requires_grad=True)

# Define the optimizer
optimizer = optim.Adam([lambda_0, lambda_2], lr=0.001)
print("kap_2 = ", kap_2)
# Optimization loop
