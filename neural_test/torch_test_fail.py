...
def q_tau(tau):
    return (2.0 * alpha_s / (3 * pi * (tau + eps))) * (-4 * np.log(tau + eps ))
#* np.exp(- 2.5 * alpha_s / (3 * pi) * (2 * np.log(tau + eps)**2))
...
# Optimizer
optimizer = optim.Adam(list(model.parameters()) + [lambda_0, lambda_2], lr=0.01)
...
for step in range(num_steps):
    ...
    total_loss =  loss + torch.abs(lambda_0 * constraint_1)  + torch.abs(lambda_2 * constraint_3)
